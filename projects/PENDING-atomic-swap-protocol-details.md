...
- **Claim 2 (BTC-locks-first by construction)**: PARTIALLY CONTRADICTED in operation-ordering — order is driven by draining-attack economics, not by the primitive (Gugger 2020 has XMR-locks-first in operation order, but still uses Bitcoin-side script for the cancel primitive). **However, a true XMR-first protocol with Monero-side cancel primitive is not deployable on the current Monero protocol** per Monero core's 2026-05-10 statement that no scheme has been specified, and the eigenwallet team's removal of the XMR-first chapter from their protocol paper on 2025-11-04. See [[projects/atomic-swap-protocol-details]], [[projects/xmr-first-atomic-swaps]], and [[projects/xmr-first-required-monero-features]].
