...
**Why this is a hard limitation, not a design choice — and not deployable on current Monero**: the BTC→XMR direction works because Bitcoin has script (timelock + 2-of-2 multisig + adaptor-signature primitive ECDSA-one-time-VES) and Monero has none. To run the swap XMR→BTC, the BTC seller would have to move first; Hoenisch and del Pino 2021 §4 explicitly analyses this and calls it the **"draining attack"**: a malicious taker would force the BTC seller to lock BTC, then bail at zero cost (the BTC seller pays the refund transaction fee), draining the BTC seller's working capital over many trials.

The mathematical fix would require **both** (a) adaptor signatures over Monero's CLSAG ring-signature scheme with witness extractability, and (b) a Monero-side non-interactive refund primitive (e.g. DLSAG or hidden timelocks). **Neither exists in deployed form**, and **neither is on the Monero roadmap**:

- **No peer-reviewed CLSAG-adaptor-signature primitive endorsed by Monero core** — Hoenisch and del Pino called it "work-in-progress" in 2021; five years later, no construction has been adopted.
- **Monero core team officially confirmed (2026-05-10) no scheme has been specified** for using Monero's on-chain primitives in atomic swaps ([getmonero.org Deprecating Unlock Time](../sources/2026-05-22-getmonero-org-deprecating-unlock-time.html)).
- **The eigenwallet team removed the XMR-first chapter** from their compiled protocol paper on 2025-11-04 (commit `6151734`, *"remove xmr to btc protocol"*), with the comment *"unsupported by the current Monero protocol. It will require a hardfork to work."* ([archived](../sources/2026-05-22-github-eigenwallet-protocol-xmr-btc.tex), lines 60-62).
- **FCMP++ (mid-2026 hardfork)** moves in the opposite direction: it deprecates `unlock_time` and does not add adaptor-signature primitives.

See [[projects/xmr-first-atomic-swaps]] for the full implementation-status survey and [[projects/xmr-first-required-monero-features]] for the detailed analysis of which Monero hardfork or feature would be required (and why none is queued). See [[projects/atomic-swap-protocol-details]] for the locks-first/draining-attack analysis.

**Practical consequence for eigenwallet users**: a user holding XMR and wanting BTC has no atomic-swap option. They must use a centralised exchange, a Haveno-style multisig-with-arbitrator DEX, or a middle-chain settlement like [[projects/serai]]. This will remain true for a 3+ year horizon at minimum.
