...
## BTC↔XMR atomic swaps are one-directional, and XMR-first is not deployable

A specific structural gap worth calling out separately from the broader middle-chain-vs-atomic-swap debate: **BTC↔XMR atomic swaps only work in the BTC-first direction**. Every deployed atomic-swap implementation ([[projects/eigenwallet]], [[projects/comit]] `xmr-btc-swap`, Farcaster, the discontinued [[projects/samourai-wallet]] GUI, community forks) ships only BTC-locked-first. A user holding XMR and wanting BTC has **no atomic-swap option**, and this will remain true for a 3+ year horizon:

- **Monero core team officially stated (2026-05-10)**: *"It is a common misconception that Monero's Unlock Time is useful for known atomic swap or payment channel protocols. To date, no scheme has been specified that utilizes Monero's Unlock Time feature."* ([getmonero.org Deprecating Unlock Time](https://www.getmonero.org/2026/05/10/deprecating-unlock-time.html), accessed 2026-05-22)
- **The eigenwallet team removed the XMR-first chapter** from their compiled protocol paper on 2025-11-04 (commit `6151734`, *"remove xmr to btc protocol"*), with the comment *"unsupported by the current Monero protocol. It will require a hardfork to work."*
- **The upcoming FCMP++ hardfork (mid-2026)** deprecates `unlock_time` rather than extending Monero's on-chain expressiveness, and does not add adaptor-signature primitives. Monero's roadmap moves *away from* the primitives an XMR-first atomic swap would need.
- **No peer-reviewed CLSAG-compatible adaptor-signature primitive** with witness extractability has been published and adopted by the Monero project in the five years since Hoenisch–del Pino 2021 §4 flagged it as a dependency.

**Implication for LEZ**: any design that needs XMR-sell-side liquidity must use one of: a middle-chain settlement model ([[projects/serai]]), Haveno-style multisig with arbitrators, a TEE-based construction, or a centralised broker ([[projects/baltex]]). None of these is an atomic swap; each carries its own trust model. **Do not propose XMR-first BTC↔XMR atomic swaps in any LEZ spec or roadmap** — the cryptographic primitives do not exist, the Monero protocol does not support them, and the Monero project is moving in the opposite direction. Full analysis in [[projects/xmr-first-atomic-swaps]] and [[projects/xmr-first-required-monero-features]].
